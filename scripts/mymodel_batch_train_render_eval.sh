...
# 数据集路径 (Linux路径)
DATA_BASE="/data/zdw/zdw_data/3dgs data/image&sparse"
MODEL_BASE="/data/zdw/zdw_data/3dgs data/my_pre-trained_models"
OUTPUT_BASE="/data/zdw/zdw_data/3dgs data/MesonGS/mymodel_output"
CSV_BASE="/data/zdw/zdw_data/3dgs data/MesonGS/mymodelexp_data"
...
ITERS=5000                    # 微调迭代次数
CONFIG="config4"          # 配置文件
TEST_ITERS="0 2000 4000 $ITERS"     # 测试迭代（0=初始评估，ITERS=最终评估）
...
for SCENE in "${SCENES[@]}"; do
    ...
    # 使用eval执行命令并捕获退出状态
    CUDA_VISIBLE_DEVICES=$CUDA_DEVICE bash -c "$RENDER_CMD" 2>&1 | tee "$RENDER_LOG"
